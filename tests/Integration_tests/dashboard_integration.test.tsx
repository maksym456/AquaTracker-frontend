...
describe('Dashboard Integration Tests', () => {
  it('opens settings panel when clicking the Settings button', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('0.19.0')).toBeInTheDocument(); // APP_VERSION z kodu
      expect(screen.getByRole('button', { name: /auth.logout/i })).toBeInTheDocument();
    });
  });
  ...
  it('calls signOut when clicking Logout button in settings panel', async () => {
    ...
    const settingsTrigger = screen.getByText('⚙️');
    fireEvent.click(settingsTrigger);
    await waitFor(() => expect(screen.getByRole('button', { name: /auth.logout/i })).toBeInTheDocument());
